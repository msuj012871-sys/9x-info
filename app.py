...
def get_account_credentials(region: str) -> str:
    r = region.upper()
    if r == "IND":
        return "uid=4381174517&password=MG-IND_CDANH_BY_MG_AQEEL_A4K5A"
    elif r == "BD":
        return "uid=4569574716&password=sumon_6SG03_BY_SPIDEERIO_GAMING_SUVZ8"
    elif r in {"BR", "US", "SAC", "NA"}:
        return "uid=4416486487&password=22_JQIO0_BY_MG_AQEEL_XZSW8"
    else:
        return "uid=4381308861&password=ARAB_5Y49L_BY_MG_AQEEL_2HR8Q"
...
async def fetch_region_from_uid(uid: str) -> str:
    ...
    url = f"https://mg-bancheck.vercel.app/bancheck?uid={uid}"
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(url, timeout=10)
            data = resp.json()
